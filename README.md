Домашні завдання з курсу "Аналітик даних в ІТ":
"Notebook 1.ipynb" - М8.ДЗ.Запуск першого ноутбуку
"hw_9.1 Cинтаксис Python.ipynb" - М9.ДЗ.Cинтаксис Python
"hw_9.1 Операції над даними.ipynb" - М9.ДЗ.Операції над даними
"hw_9.2 Списки.ipynb" - М9.ДЗ.Списки
"hw_9.2 Словники, набори, кортежі.ipynb" - М9.ДЗ.Словники, набори, кортежі
"hw_9.2 Control Flow.ipynb" - М9.ДЗ.Control Flow
"hw_9.3 Цикл for.ipynb" - М9.ДЗ.Цикл for
"hw_9.3 Comprehensions.ipynb" - М9.ДЗ.Comprehension в Python
"hw_9.3 Цикл while.ipynb" - М9.ДЗ.Цикл while
"hw_9.4 Функції. Основи.ipynb" - М9.ДЗ.Функції.Основи
"hw_9.4 Функції.Продовження.ipynb" - М9.ДЗ.Функції.Продовження
"hw_9.5 Модулі.Винятки.ipynb" - М9.ДЗ.Модулі.Винятки
"hw_9.5 Класи.ipynb" - М9.ДЗ.Класи
"hw_10.1 Введення в Pandas та NumPy.ipynb" - М10.ДЗ.Введення у Pandas та Numpy
"hw_10.2 Основи роботи з даними в Pandas.ipynb" - М10.ДЗ.Основи роботи з даними у Pandas
"hw_10.3 Розширені методи обробки даних у Pandas.ipynb" - М10.ДЗ.Розширені методи обробки даних у Pandas
"hw_10.4 Apply, groupby, pivot_table.ipynb" - М10.ДЗ.Apply, groupby, pivot_table
"hw_11.1 Візуалізація_даних_з_Pandas.ipynb" - М11.ДЗ.Візуалізація даних з Pandas
"hw_11.2 Візуалізація даних з Matplotlib.ipynb" - М11.ДЗ.Візуалізація даних з Matplotlib
"hw_11.3 Статистичні візуалізації з Seaborn.ipynb" - М11.ДЗ.Статистичні візуалізації з Seaborn
"hw_12.1 Інтеграція Python та SQL_ запити даних.ipynb" -  М12.ДЗ.Інтеграція Python та SQL: запити даних
"hw_12.2 Внесення оновлень у БД і робота з транзакціями.ipynb" - М12.ДЗ.Внесення оновлень в БД і робота з транзакціями
"hw_12.3 Робота з АРІ та створення ETL-пайплайну.ipynb" - М12.ДЗ.ETL-пайплайни для аналітиків даних
"hw_15.2 Перевірка статичних гіпотез. Z-test.ipynb" - М15.ДЗ.Перевірка статиcтичних гіпотез. Z-test
"hw_15.3 Тести для малих вибірок та пропорцій.ipynb" - М15.ДЗ.Тести для малих вибірок та пропорцій
"hw_15.4 Аналіз А/В-тестів.ipynb" - М15.ДЗ.Аналіз А/В-тестів
"hw_16.1 Прогнозування орендної плати за житло.ipynb" - М16.ДЗ.Прогнозування орендної плати за житло
"hw_16.2 Прогнозування кредитного ризику.ipynb" - М16.ДЗ.Задача класифікації
"hw_17.1 Чистий код.ipynb" - М17.ДЗ.Чистий код та оптимізація аналітичної роботи
